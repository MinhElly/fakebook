        ...
        public void accept(Message<String> message) {
            try {
                ...
                if (!jsonNode.has("authorId") || jsonNode.get("authorId").isNull()) {
                    ...
                    String status = jsonNode.has("status") && !jsonNode.get("status").
  isNull() ? jsonNode.get("status").asString() : "ACTIVE";
                    String visibility = jsonNode.has("visibility") && !jsonNode.
  get("visibility").isNull() ? jsonNode.get("visibility").asString() : "PUBLIC";
                ...
                // Gọi Manual Ack xác nhận đã xử lý xong
