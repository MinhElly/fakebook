...
import com.minh.fakebook.auth.IntegrationTest;
...
class AuthServiceKafkaResourceIT {
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/auth-service-kafka/publish?message=value-produce").with(csrf())).andExpect(status().isOk());
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/auth-service-kafka/register"))
        ...
        for (int i = 0; i < 100; i++) {
            ...
            if (content.contains("data:value-consume")) {
                restMockMvc.perform(get("/api/auth-service-kafka/unregister"));
                return;
            }
        }
