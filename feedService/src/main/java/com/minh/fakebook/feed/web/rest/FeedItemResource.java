...
import com.minh.fakebook.feed.service.FeedItemQueryService;
import com.minh.fakebook.feed.service.FeedItemService;
import com.minh.fakebook.feed.service.criteria.FeedItemCriteria;
import com.minh.fakebook.feed.service.dto.FeedItemDTO;
import com.minh.fakebook.feed.web.rest.errors.BadRequestAlertException;
...
/**
 * REST controller for managing {@link com.minh.fakebook.feed.domain.FeedItem}.
 */
@RestController
...
public class FeedItemResource {
    ...
    private static final String ENTITY_NAME = "feedServiceFeedItem";

    @Value("${jhipster.clientApp.name:feedService}")
    ...
    private final FeedItemQueryService feedItemQueryService;

    public FeedItemResource(
        FeedItemService feedItemService,
        FeedItemRepository feedItemRepository,
        FeedItemQueryService feedItemQueryService
    ) {
        this.feedItemService = feedItemService;
        this.feedItemRepository = feedItemRepository;
        this.feedItemQueryService = feedItemQueryService;
    }
    ...
    /**
     * {@code GET  /feed-items} : get all the Feed Items.
     *
     * @param pageable the pagination information.
     * @param criteria the criteria which the requested entities should match.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the list of Feed Items in body.
     */
    @GetMapping("")
    public ResponseEntity<List<FeedItemDTO>> getAllFeedItems(
        FeedItemCriteria criteria,
        @org.springdoc.core.annotations.ParameterObject Pageable pageable
    ) {
        LOG.debug("REST request to get FeedItems by criteria: {}", criteria);

        Page<FeedItemDTO> page = feedItemQueryService.findByCriteria(criteria, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }

    /**
     * {@code GET  /feed-items/count} : count all the feedItems.
     *
     * @param criteria the criteria which the requested entities should match.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the count in body.
     */
    @GetMapping("/count")
    public ResponseEntity<Long> countFeedItems(FeedItemCriteria criteria) {
        LOG.debug("REST request to count FeedItems by criteria: {}", criteria);
        return ResponseEntity.ok().body(feedItemQueryService.countByCriteria(criteria));
    }
